...
export const getMenu = async(req,res) =>{
    ...
    try {
        let menu = await pool.query(
            `select * from menu where hostel_name = $1` ,
            ['SVBH'||req.user.hostel]
        );
        if (menu.rowCount===0) {
            console.log("no menu found in db")
            return res.status(404).json({status:false,error:"menu not found"});
        }
    menu  = menu.rows;
    console.log(menu);
...
export const getNotices = async (req, res) => {
    try {
      ...
      console.error(error);
      res.status(500).json({status:false, message: "Failed to fetch notices" });
    }
  };
